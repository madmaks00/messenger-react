...
// Кэш для измерения высоты текста
...
function getMeasureContext(): CanvasRenderingContext2D | null {
  if (!measureCtx && typeof document !== 'undefined') {
    ...
    if (measureCtx) {
      measureCtx.font = '15px "Segoe UI", -apple-system, BlinkMacSystemFont, Roboto, sans-serif';
    }
  }
  return measureCtx;
}

function calculateTextLinesCount(text: string, maxWidth: number): number {
  if (!text) return 0;
  const ctx = getMeasureContext();
  if (!ctx || maxWidth <= 0) {
    return Math.max(1, Math.ceil(text.length / 35));
  }
...
export class AsyncChatLayoutEngine {
  /**
   * Точный алгоритм расчета размеров медиа из C# (CalculateMediaDimensionsFromPixels)
   */
  ...
  /**
   * Размеры сетки альбома (CalculateAlbumDimensions)
   */
  ...
  /**
   * Полный маппинг сырых IMessage в IMessageLayoutModel (с учетом реплаев, медиа и звонков)
   */
  public static createLayoutModels(
    ...
    return messages.map((m) => {
      ...
      return {
        ...
        isMyMessage: m.isMyMessage || m.senderId === currentUserId,
  ...
  /**
   * Асинхронный расчет координат и высот всех сообщений
   * (эквивалент CalculateLayoutAsync)
   */
  public static calculateLayout(
    ...
    for (const msg of messages) {
      ...
      if (msg.isCallMessage) {
        ...
        if (hasText) {
          const lines = calculateTextLinesCount(msg.text, maxBubbleWidth - 30);
          textContentHeight = lines * 20.0;
        }
        ...
        if (hasDocs) {
          bubbleHeight += (msg.documents.length * 163.0 + 20.0) / 3.0;
        }
        ...
        if (!msg.isMyMessage && msg.isGroupMessage && hasMedia && (hasDocs || hasAudios)) {
          bubbleHeight += 0.5;
        }

        if (!msg.isForwarded && !msg.isDeletedForMe && (msg.isChannel || (!msg.isMyMessage && msg.isGroupMessage))) {
          if ((hasDocs || hasAudios) && !hasMedia && !hasText) bubbleHeight += 31.0;
          else if (hasMedia && !hasText) bubbleHeight += msg.previewMedia.length >= 4 ? 32.0 : 31.0;
          else bubbleHeight += 23.5;
        }

        if (msg.isForwarded && !msg.isDeletedForMe) {
          if (hasMedia && (hasDocs || hasAudios)) bubbleHeight += 39.0;
          else if (hasMedia && !hasText && !hasDocs && !hasAudios) bubbleHeight += 44.5;
          else bubbleHeight += 38.0;
        }
